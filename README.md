...
In case of problem with the InApp delivery backend, the system still sets the http resonse status code 200 and returns the qrCode and the PDF. 
Additionally, the response contains the optional attribute **appDeliveryError**. 
If this attribut exists on the response, then we could not transfer the certificate to the Covid Cert app. 
...
- `errorCode:558, "errorMessage": "App delivery failed due to a technical error."`
  This is a technical issue on our side akin to a 500 http response status code.
  You can try to submit the same request later. Please use some backoff time.
